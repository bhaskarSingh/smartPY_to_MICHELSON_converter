...
class Test:
    ...
    def eval(self):
        ...
        smartpy.profile("end")
        if self.profile:
            window.addOutput("<hr/><h4>Profiling</h4>" + "<br>".join(smartpy.sp.profilingLogs))
...
def showTraceback(title, trace):
    ...
    for line in trace.split('\n'):
        ...
        if skip:
            skip = False
            continue
        skip = (('module smartpy line' in line and ('in runScenario' in line or 'in pp' in line))
                or ('module smartpyio line' in line and ('in run' in line or 'in eval' in line or 'in toException' in line))
    ...
    for line in lines:
        ...
        if line[newIndent:].startswith('sp.for '):
            ...
            if nline[-1] == ':' and p[0] == 'sp.for' and p[2] == 'in':
                line = "%swith sp.for_('%s', %s) as %s:" % (newIndent * ' ', p[1], ' '.join(p[3:]), p[1])
...
def run(withTests):
    ...
    for change in changes:
        if change[0] in code:
            if window.inBrowser:
                message = ('Warning: syntax change: %s -> %s' % (change[0], change[1]) +
                           "\n\nMigrate Syntax to adapt your script automatically?\n\nOther changes:\n" +
                           '\n'.join("%s -> %s" % (change[0], change[1]) for change in changes))
                ...
                print('Warning: syntax change: %s -> %s. You can use the editor to adapt it.' % (change[0], change[1]))
    ...
    if withTests and len(window.pythonTests) == 0:
        ...
        for c in env:
            ...
            if hasattr(env[c], 'collectMessages'):
                html += "There is a sp.Contract class '%s' but no test is defined.\n\nPlease add a test such as:\n%s" % (str(c), testTemplate % (c, c))
        if html:
            alert(html)
